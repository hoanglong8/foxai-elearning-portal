...
export default async function DashboardPage() {
  let profile = { full_name: 'Nguyễn Hoàng Long' }
  ...
  if (!isDemoMode) {
    ...
    const { data: e } = await supabase
      .from('enrollments').select('course_id, enrolled_at, completed_at').eq('user_id', user.id)
    const { data: pr } = await supabase
      .from('user_progress').select('lesson_id, course_id, completed').eq('user_id', user.id)
  ...
  const enrolledCourses = COURSES.filter((c) => enrolledCourseIds.has(c.id)).map((c) => {
    const completed = c.lessons.filter((l) => completedLessonIds.has(l.id)).length
    const total = c.lessons.length
    const pct = total > 0 ? Math.round((completed / total) * 100) : 0
    return { ...c, completed, total, pct }
  })
    ...
    <div className="p-8 max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">
                {COURSES.reduce((s, c) => s + c.lessons.reduce((a, l) => a + l.duration_minutes, 0), 0)} phút
              </p>
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl p-6 text-white">
        ...
        <p className="text-blue-100 text-sm mb-4">
          {COURSES.length} khóa học từ AI, Kỹ thuật đến Sales & Quy trình nội bộ FOXAI.
        </p>
        <Link
          href="/courses"
